    ...
    class Controller{
		...
		function build_view($html_topbar,$html_side,$html_content){
			...
			$html_view =  str_replace(
					...
					(isset($this->view) && !empty($this->view->get_script_js()) ? 
        ...
        function render($filename,$auto_build=false){
			...
			if(Session::get('log_out')){
				...
				$this->view_processor->set_topbar(
					CoreUtils::get_layout_template_content('topbar',$this->layout));
				$this->view_processor->set_sidebar(
					CoreUtils::get_layout_template_content('side',$this->layout));
//TODO en proceso
				if($auto_build){
					if($filename == 'index'){
						$html_content = 
					...
					//echo "<script>console.log('get_view_file_content(filename,this)')</script>";
					...
					if($html_content==''){
						...
						//echo "<script>console.log('get_view_file_content(index,indexController)')</script>";
					}else{
						//echo "<script>console.log('html_content!=null')</script>";
						if(isset($this->view)){
							//echo "<script>console.log('acciones de vista iniciada')</script>";
		...
		*/
		public function init($obj){
			...
			$this->view_processor = new ViewPocessor();
		}
