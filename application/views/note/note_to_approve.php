...
function generate_button_approve($note_id, $user_id){

    $disable=Model::get_sql_data("SELECT 1 as value from note_approver na inner JOIN note n on (n.id=na.note_id) 
    inner join `user` u on (u.id=na.user_id) 
    where na.note_id=? and na.user_id=? and choice!=''", array('note_id'=>$note_id,'user_id'=>$user_id));
	$disable =empty($disable['value'])?'':'disabled';
